...
export default function EditArticlePage({ params }: { params: { id: string } }) {
  ...
  // Unwrapping params for Next 15+ compatibility
  ...
  const [excerpt, setExcerpt] = useState('This is a mocked excerpt for the article editor.');
  ...
  const handleSave = async (status: 'DRAFT' | 'REVIEW') => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      toast.success(`Article ${status === 'DRAFT' ? 'saved as draft' : 'submitted for review'}`);
      router.push('/dashboard/articles');
    }, 1000);
  };

  if (!resolvedParams) {
    return <div className="p-8 text-center text-muted-text">Loading editor...</div>;
  }
    ...
    <div className="max-w-5xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between mb-8 sticky top-0 bg-warm-white z-20 py-4 border-b border-pearl-soft">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-dark-text tracking-tight">Edit Article ({resolvedParams.id})</h1>
        </div>
        ...
        <div className="w-full lg:w-80 flex-shrink-0 space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="w-full aspect-[16/9] bg-warm-white border-2 border-dashed border-pearl-soft rounded-md flex flex-col items-center justify-center cursor-pointer hover:bg-pearl-light hover:border-pearl-red transition-all">
              ...
              <span className="text-sm font-bold text-pearl-red">Change Image</span>
            </div>
          </div>
        </div>
      </div>
